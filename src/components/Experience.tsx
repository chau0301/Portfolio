...
export default function Experience(experienceProps: ExperienceProps) {
  const { time, name, description, stacks } = experienceProps.experience;
    ...
    <div className="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
      ...
      <div className="z-10 sm:col-span-6">
        ...
        {description.map((paragraph, index) => (
          <p className="mt-2 text-sm leading-normal text-secondary" key={`p-${index}`}>
            {paragraph}
          </p>
        ))}
